...
public class xTCSParticles {
    ...
    protected double elec_chi2, posi_chi2, nucl_chi2; // pid chi2 values
    ...
    protected double nucl_px, nucl_py, nucl_pz, nucl_p, nucl_e, nucl_theta, nucl_phi; // nucleon kinematics
    ...
    public static boolean channel_test(xTCSParticles variables) {
    ...
    public xTCSParticles(DataEvent event, PhysicsEvent recEvent, double Eb, int nuclPID_int, String nuclPID_str) {
        ...
        int nucl_rec_index = getIndex(rec_Bank, nuclPID_int, 0);

        elec_chi2 = rec_Bank.getFloat("chi2pid", elec_rec_index);
        posi_chi2 = rec_Bank.getFloat("chi2pid", posi_rec_index);
        nucl_chi2 = rec_Bank.getFloat("chi2pid", nucl_rec_index);
        ...
        // nucleon detector
        if (generic_tests.forward_tagger_cut(nucl_rec_index, rec_Bank)) {
            nucl_detector = 0; // Forward Tagger
        } else if (generic_tests.forward_detector_cut(nucl_rec_index, rec_Bank)) {
            nucl_detector = 1; // Forward Detector
        } else if (generic_tests.central_detector_cut(nucl_rec_index, rec_Bank)) {
            nucl_detector = 2; // Central Detector
        }

        // Set up Lorentz vectors
        // target
        LorentzVector target_lv = new LorentzVector();
        target_lv.setPxPyPzM(0,0,0,kinematic_variables.particle_mass(nuclPID_int));
        ...
        // nucleon
        String nucleon_string = "["+nuclPID_str+",0]";
        Particle scattered_nucleon = recEvent.getParticle(nucleon_string);
        LorentzVector nucl_lv = new LorentzVector();
        nucl_lv.setPxPyPzM(scattered_nucleon.px(), scattered_nucleon.py(),
                           scattered_nucleon.pz(), kinematic_variables.particle_mass(nuclPID_int));

        // positions of electron, positron, nucleon
        elec_vx = scattered_electron.vx();
        posi_vx = scattered_positron.vx();
        nucl_vx = scattered_nucleon.vx();
        elec_vy = scattered_electron.vy();
        posi_vy = scattered_positron.vy();
        nucl_vy = scattered_nucleon.vy();
        elec_vz = scattered_electron.vz();
        posi_vz = scattered_positron.vz();
        nucl_vz = scattered_nucleon.vz();
        ...
        // kinematics of nucleon
        nucl_px    = nucl_lv.px();
        nucl_py    = nucl_lv.py();
        nucl_pz    = nucl_lv.pz();
        nucl_p     = nucl_lv.p();
        nucl_e     = nucl_lv.e();
        nucl_theta = scattered_nucleon.theta();
...
//        System.out.println("runnum not found, assigning helicity flip");
    ...
    public int get_nucl_detector() {
        return nucl_detector;
    } // returns integer value representing the detector of nucleon track
    ...
    public double get_nucl_chi2pid() {
        return nucl_chi2;
    } // returns chi2 value of nucleon pid
    ...
    public double get_nucl_px(){
        return nucl_px;
    } // returns nucleon px

    public double get_nucl_py() {
        return nucl_py;
    } // returns nucleon py

    public double get_nucl_pz(){
        return nucl_pz;
    } // returns nucleon pz

    public double get_nucl_p() {
        return nucl_p;
    } // returns nucleon p

    public double get_nucl_e() {
        return nucl_e;
    } // returns nucleon e

    public double get_nucl_theta() {
        return nucl_theta;
    } // returns nucleon theta

    public double get_nucl_phi() {
        return nucl_phi;
    } // returns nucleon phi
    ...
    public double get_nucl_vx() {
        return nucl_vx;
    } // returns nucleon vx

    public double get_nucl_vy() {
        return nucl_vy;
    } // returns nucleon vy
