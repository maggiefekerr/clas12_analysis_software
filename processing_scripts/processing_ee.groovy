...
public static void main(String[] args) {
    ...
    String output_file = args.length < 2 ? "ee_dummy_out.txt" : args[1]
    if (args.length < 2) println('WARNING: Specify an output file name. Set to "ee_dummy_out.txt".')
    ...
    int elec_detector, posi_detector;
	double elec_chi2, posi_chi2;
	...
	GenericKinematicFitter fitter = new ee_fitter(10.6041);

    // set filter for final states
	EventFilter filter = new EventFilter("11:-11:X+:X-:Xn");
    ...
    for (current_file in 0..<n_files) {
        ...
        while (reader.hasEvent()) {
            ...
            if (process_event) {
                ...
                eeParticles variables = new eeParticles(event, research_Event, energy);
