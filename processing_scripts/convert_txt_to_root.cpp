...
double getPol(int runnum) {
  double pol = 0.83; 
    ...
    else if (runnum >= 16043 && runnum < 16772) { pol = 0.8384; } // RGC Su22
    else if (runnum >= 16843 && runnum < 17408) { pol = 0.8372; } // RGC Fa22
    else if (runnum >= 17477 && runnum < 17811) { pol = 0.8040; } // RGC Sp23
  return pol;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        ...
        cout << " <script_index> = 10 for three particles w/o cuts" << endl;
        return 1;
    }
    ...
    tree->SetMaxTreeSize(200000000000LL);
    ...
    if (script_index == 9 && is_mc == 0) {
        ...
        tree->Branch("p_dc_fid_cut", &p_dc_fid_cut, "p_dc_fid_cut/I");
    }
    // Case for two hadrons (dihadrons) without quality cuts
    else if (script_index == 10 && is_mc == 0) {
        // Link TTree branches to variables for two hadrons
        tree->Branch("fiducial_status", &fiducial_status, "fiducial_status/I");
        tree->Branch("runnum", &runnum, "runnum/I");
        tree->Branch("num_pos", &num_pos, "num_pos/I");
        ...
        tree->Branch("e_pcal_fid_cut", &e_pcal_fid_cut, "e_pcal_fid_cut/I");
        tree->Branch("e_dc_fid_cut", &e_dc_fid_cut, "e_dc_fid_cut/I");
        tree->Branch("p1_chi2pid_cut", &p1_chi2pid_cut, "p1_chi2pid_cut/I");
        tree->Branch("p1_vertex_cut", &p1_vertex_cut, "p1_vertex_cut/I");
        tree->Branch("p1_dc_fid_cut", &p1_dc_fid_cut, "p1_dc_fid_cut/I");
        tree->Branch("p1_cvt_fid_cut", &p1_cvt_fid_cut, "p1_cvt_fid_cut/I");
    ...
    if (script_index == 7 && is_mc == 0) {
        while (infile >> num_pos >> num_neg >> num_neutral >> 
            ...
            double nucl_px, nucl_py, nucl_pz;
            sphericalToCartesian(nucl_p, nucl_phi, nucl_theta, nucl_px, nucl_py, nucl_pz);
    ...
    // Two Hadrons, no quality cuts
    if (script_index == 10 && is_mc == 0) {
        while (infile >> fiducial_status >> num_pos >> num_neg >> num_neutral >>
            runnum >> evnum >> helicity >> detector1 >> detector2 >> e_p >> e_theta >> e_phi >> vz_e >> 
            p1_p >> p1_theta >> p1_phi >> vz_p1 >> p2_p >> p2_theta >> p2_phi >> vz_p2 >> 
            open_angle_ep >> open_angle_ep1 >> open_angle_ep2 >> open_angle_p1p2 >> 
