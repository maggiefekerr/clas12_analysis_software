...
def asy(directory, name, Q2, xB, t, beamE, numBins):
    phiDivs = []
    ...
    for i in range(numBins):
        ...
        pt.prepare()
        aluList.append(float(th_KM15.predict(pt)))

    file = "./ratios/data/" + directory + "/asy/-t_" + str(t) + "/" + name + ".txt"
    with open(file, "w") as f:
        for i in range(numBins):
            if i != (numBins - 1):
                f.write(str(phiList[i]) + " " + str(aluList[i]) + '\n')
            else:
                f.write(str(phiList[i]) + " " + str(aluList[i]))
    f.close()
...
if __name__ == "__main__":
    ...
    if method == "asy":
        asy(str(a), str(b), float(c), float(d), float(e), float(f), int(g))
    else:
        print(":(")